...
class StockReconciliation(StockController):
        ...
        def on_submit(self):
                ...
                self.make_custom_gl_entry()
                self.repost_future_sle_and_gle()

        def make_custom_gl_entry(self):
                """Create a single GL Entry with credit and debit set to self.difference_amount on self.expense_account."""
                if not self.difference_amount or not self.expense_account:
                        return

                gle = frappe.new_doc("GL Entry")
                gle.account = self.expense_account
                gle.company = self.company
                gle.posting_date = self.posting_date
                gle.voucher_type = self.doctype
                gle.voucher_no = self.name

                if self.difference_amount < 0:
                        gle.debit = abs(self.difference_amount)
                        gle.credit = 0
                else:
                        gle.credit = self.difference_amount
                        gle.debit = 0

                gle.cost_center = self.cost_center
                gle.remarks = _(f"Custom GL Entry for Stock Reconciliation {self.name}")
        ...
        def calculate_difference_amount(self, item, item_dict):
                ...
                self.difference_amount += (new_qty * new_valuation_rate) - (current_qty * new_valuation_rate)
